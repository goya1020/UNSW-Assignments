...
% 
next_arrival_time = round((-log(1-rand(1))/lambda)* ((1.17-0.75)*rand()+0.75),1);
service_time_next_arrival = round(nthroot((rand()/coef+alpha1^0.14),0.14) / (1.25+0.31*((2000/s)/200 - 1)),1);
...
while (N<5)
    ...
    disp('                    master_clock                next_event_type            next_arrival_time        next_departure_time')
    disp([master_clock, next_event_type, next_arrival_time, next_departure_time]);
    disp('job list is below')
    disp(job_list)
    disp('------------------------------------------------------------------------------------------------------------------------------------')
    %     
    % update master clock
    ...
    if (next_event_type == 1) % an arrival 
        ...
        jump_job_list = 0;
        for i=1:s
            jump_job_list = jump_job_list + round((-log(1-rand(1))/lambda)* ((1.17-0.75)*rand()+0.75), 1);
        end
