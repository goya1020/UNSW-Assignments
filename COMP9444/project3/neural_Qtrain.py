...
NUM_EPISODES = 600  # Episode limitation # 850
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    logits1_tanh = tf.nn.tanh(logits1)  # [?, 16]

    # second layer
    hidden2 = 20
    w2 = weights([hidden_nodes, hidden2])  # [?, 16] mul [16, 32] is [?, 32]
    b2 = bias([hidden2])
    logits2 = tf.matmul(logits1_tanh, w2) + b2
    ...
    # # attempt for Convolutional NN

    # # first tanh layer
    # w1 = weights([state_dim, hidden_nodes])
    # b1 = bias([hidden_nodes])
    # logits1 = tf.matmul(state_in, w1) + b1
    # logits1_tanh = tf.nn.tanh(logits1)  # [?, 64]

    # # second conv layer
    # width = height = 15
    # filter_shape = (4, 4)
    # convlayer_sizes = 8
    # reshped_logits = tf.reshape(logits1_tanh, [-1, width, height, 1])
    # w2 = weights([filter_shape[0], filter_shape[1], 1, convlayer_sizes])
    # b2 = bias([convlayer_sizes])
    # conv = conv2d(reshped_logits, w2) + b2 # (???, 8, 8, 10)
    # # conv_dropout = tf.nn.dropout(conv, keep_prob=0.7)

    # conv_flat = tf.reshape(conv, [-1, 512])
    # w = weights([512, action_dim])
    # b = bias([action_dim])
    # q_values = tf.matmul(conv_flat, w) + b  # [?, action_dim]
    ...
    optimise_step = tf.train.AdagradDAOptimizer(
        0.1, global_step=gs).minimize(loss)
    # optimise_step = tf.train.AdagradOptimizer(0.01).minimize(loss)
...
def do_train_step(replay_buffer, state_in, action_in, target_in,
    ...
    # print(len(target_batch), len(state_batch), len(action_batch))
